...
sim_data <- extract_sim_data(sim_data = raw_data,
                             sim_names = sim_names,
                             sim_ids = sim_ids)
```
...
my_peaks <- find_peaks(sim_data = raw_data,
                       sim_ids = sim_ids,
                       sim_names = sim_names,
                       noise = noise)
```
...
```{r}
my_peaks <- group_peaks(peak_data = my_peaks,
                        rt_diff = 5)
```
...
Calculate relative intensity for each peak group. The sum of all peak traces is 100%.

```{r}
my_peaks_clean <- my_peaks_clean %>% 
  group_by(peak_group) %>% 
  mutate(int_perc = maxo / sum(maxo) * 100,
         int_ratio = maxo / sum(maxo))
```
...
```{r fig.width=7, fig.height=4}
my_peaks_clean %>% 
  filter(peak_group == 19) %>% 
  mutate(sim_mz = as.numeric(as.character(sim))) %>% 
  ggplot() +
  geom_linerange(aes(x = sim_mz,
                     ymin = 0,
                     ymax = int_perc)) +
  scale_x_continuous(breaks = seq(20, 260, 20),
                     limits = c(20, 260)) +
  labs(x = expression(italic("m/z")),
       y = "Relative intensity [%]",
       subtitle = "Reconstructed EISA fragmentation spectrum",
       title = "Cystine") +
  theme_minimal()
```
